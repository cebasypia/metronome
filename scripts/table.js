...
const addRow = (obj = {}, bar) => {
  const rows = Array.from(tBodyElement.rows)
  const tHead = Array.from(tableElement.rows)
  console.log(rows)
  rows.push(tBodyElement.insertRow(-1))

  for (let i = 0; i < tHead[0].cells.length; i++) {
    const cell = rows[rows.length - 1].insertCell()
    let value = ''
...
console.log(makeTableFromJSON, makeJSONFromTable)
